...
const logger = bunyan.createLogger({ name: "views/myEvents" });
const router: express.Router = express.Router();
const currPath = "/myEvents";
const viewPath = "pages/myEvents";
...
router.get("/", async (req, res) => {
	...
	const myEvents = await db.run(query) as EventModel[];
...
router.get("/:eventID", async (req, res) => {
	...
	const myEvents = await db.run(query, [eventID]) as EventModel[];
